        ...
        try {
            ...
            if (usuarioLogado.getPermissao() == 1) {
                if (cadRevenda.inserir(revenda)) {
                    Revenda inserida = (Revenda) cadRevenda.pesquisarNome(nome);
                    int id_rev = inserida.getId();

                    File folder = new File("../../../web/img/" + id_rev);
                    boolean criou = folder.mkdirs();
                    if (!criou) {
                        msg = "Inserido com sucesso, porém não criou a pasta! Contate um Administrador";
                    } else {
